...
- 新增 VIP 課長等級，排在 ㄅ 之上（最高級），排行榜以 👑 標記。
- VIP 輸入不分大小寫（vip／Vip／VIP），一律以大寫 `VIP` 儲存與顯示。
...
- 每顆巨菇限制 3～5 位玩家。
- 每位玩家只要參與某顆巨菇，至少派遣 1000 戰力（常數 `MIN_POWER_PER_MUSHROOM`）。
- 最佳化目標改為整體星級最大化；同分時依序比較四星、三星、二星數量。
- 新增「接近下一星級」作為最佳化排序條件（達到星級後仍會繼續分配剩餘戰力）。
- 改善活動巨菇最佳化輸出排版與可讀性。
...
課長排行榜是完全獨立的系統，使用獨立的 `whales` 資料表（不需「菇」前綴）。級分固定只有 `VIP`、`ㄅ`、`ㄆ`、`ㄇ`、`ㄈ`、`ㄦ`，排序為 VIP→ㄅ→ㄆ→ㄇ→ㄈ→ㄦ（`VIP` 為最高級，輸入不分大小寫，一律以大寫 `VIP` 儲存與顯示）。
...
- 輸入不存在的級分（例如 `ㄉ`、`A`、`S`）會回覆「⚠️ 級分只能是：VIP、ㄅ、ㄆ、ㄇ、ㄈ、ㄦ」。
...
> 最佳化目標為整體星級最大化，並在達到星級後仍持續分配剩餘戰力以更接近下一星級；每顆活動巨菇需 3～5 位玩家，且每位玩家單顆至少派遣 1000 戰力。若剩餘派遣次數不足以規劃指定顆數，Bot 會回覆目前最多可安排的顆數，例如「⚠️ 無法規劃 3 顆活動巨菇。⋯目前最多可安排：2 顆。」。
